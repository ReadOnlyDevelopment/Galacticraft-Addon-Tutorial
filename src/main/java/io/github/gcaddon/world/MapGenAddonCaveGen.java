package io.github.gcaddon.world;
...
public class MapGenAddonCaveGen extends MapGenBaseMeta {
    ...
    public MapGenAddonCaveGen() {
    	this.top = Blocks.DIAMOND_BLOCK.getDefaultState();
    	
    }

    public MapGenAddonCaveGen(Set<Block> topBlock, IBlockState lava, Set<Block> digBlock, Set<Block> fluidBlock) {
    ...
    public MapGenAddonCaveGen(IBlockState top, Set<Block> digBlock) {
    	this(top, Blocks.LAVA.getDefaultState(), digBlock, new HashSet<>());
    }

    public MapGenAddonCaveGen(IBlockState top, IBlockState lava, Set<Block> digBlock) {
        this(top, lava, digBlock, new HashSet<>());
    }

    public MapGenAddonCaveGen(IBlockState top, IBlockState lava, Set<Block> digBlock, Set<Block> fluidBlock) {
        this.top = top;
        this.lava = lava;
        this.digBlock = digBlock;
        this.fluidBlock = fluidBlock;
    ...
    private void addTunnel(long seed, int originalX, int originalZ, ChunkPrimer primer, double x, double y, double z, float p_180702_12_, float p_180702_13_, float p_180702_14_,
        ...
        for (boolean flag = rand.nextInt(6) == 0; p_180702_15_ < p_180702_16_; ++p_180702_15_) {
            ...
            if (!flag2 && p_180702_15_ == j && p_180702_12_ > 1.0F && p_180702_16_ > 0) {
                this.addTunnel(rand.nextLong(), originalX, originalZ, primer, x, y, z, rand.nextFloat() * 0.5F + 0.5F, p_180702_13_ - (float) Math.PI / 2F, p_180702_14_
                        / 3.0F, p_180702_15_, p_180702_16_, 1.0D);
                this.addTunnel(rand.nextLong(), originalX, originalZ, primer, x, y, z, rand.nextFloat() * 0.5F + 0.5F, p_180702_13_ + (float) Math.PI / 2F, p_180702_14_
                        / 3.0F, p_180702_15_, p_180702_16_, 1.0D);
                return;
            }
